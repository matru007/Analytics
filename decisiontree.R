...
table(data$survived)
str(data)
fit <- rpart(survived ~ ., data = data, method = 'class') # tilde. use all other columns other than survive
fit
rpart.plot(fit, extra = 104, cex=.8,nn=T)  #plot
head(data)
printcp(fit) #select complexity parameter
prunetree2 = prune(fit, cp=.015) # cutshot the tree #complexity parameter (used to prune the tree)
rpart.plot(prunetree2, cex=.8,nn=T, extra=104) #gd coefficient (used to find most imp variable)
prunetree2
nrow(data)
table(data$survived)
